...
namespace MaterialPrinter
{
	public class XmlReader
	{
		...
		private void get_float4(float4 fl, string floats)
		{
			if (string.IsNullOrEmpty(floats)) return;

			var vec = parse_floats(floats);
			fl = new float4(vec[0], vec[1], vec[2]);
		}
			...
			while (node.Read())
			{
				...
				switch (node.Name)
				{
					...
					case "mixrgb":
						...
						break;
					case "mixclosure":
						var mixcls = new MixClosureNode();
						get_float(mixcls.ins.Fac, node.GetAttribute("fac"));
						nodes.Add(nodename, mixcls);
						shader.AddNode(mixcls);
						break;
					case "addclosure":
						var addcls = new AddClosureNode();
						nodes.Add(nodename, addcls);
						shader.AddNode(addcls);
						break;
					case "fresnel":
						var frs = new FresnelNode();
						get_float4(frs.ins.Normal, node.GetAttribute("normal"));
						get_float(frs.ins.IOR, node.GetAttribute("ior"));
						nodes.Add(nodename, frs);
						shader.AddNode(frs);
						break;
					case "math":
						var mth = new MathNode();
						get_float(mth.ins.Value1, node.GetAttribute("value1"));
						get_float(mth.ins.Value2, node.GetAttribute("value2"));
						var operation = node.GetAttribute("type");
						if (!string.IsNullOrEmpty(operation))
						{
							if (operation.ToLower() == "absolute")
								mth.Operation = MathNode.Operations.Absolute;
							else if (operation.ToLower() == "add")
								mth.Operation = MathNode.Operations.Add;
							else if (operation.ToLower() == "substract")
								mth.Operation = MathNode.Operations.Subtract;
							else if (operation.ToLower() == "multiply")
								mth.Operation = MathNode.Operations.Multiply;
							else if (operation.ToLower() == "divide")
								mth.Operation = MathNode.Operations.Divide;
							else if (operation.ToLower() == "sine")
								mth.Operation = MathNode.Operations.Sine;
							else if (operation.ToLower() == "cosine")
								mth.Operation = MathNode.Operations.Cosine;
							else if (operation.ToLower() == "tangent")
								mth.Operation = MathNode.Operations.Tangent;
							else if (operation.ToLower() == "arcsine")
								mth.Operation = MathNode.Operations.Arcsine;
							else if (operation.ToLower() == "arccosine")
								mth.Operation = MathNode.Operations.Arccosine;
							else if (operation.ToLower() == "arctangent")
								mth.Operation = MathNode.Operations.Arctangent;
							else if (operation.ToLower() == "power")
								mth.Operation = MathNode.Operations.Power;
							else if (operation.ToLower() == "logarythm")
								mth.Operation = MathNode.Operations.Logarithm;
							else if (operation.ToLower() == "minimum")
								mth.Operation = MathNode.Operations.Minimum;
							else if (operation.ToLower() == "maximum")
								mth.Operation = MathNode.Operations.Maximum;
							else if (operation.ToLower() == "round")
								mth.Operation = MathNode.Operations.Round;
							else if (operation.ToLower() == "less_than")
								mth.Operation = MathNode.Operations.Less_Than;
							else if (operation.ToLower() == "greater_than")
								mth.Operation = MathNode.Operations.Greater_Than;
							else if (operation.ToLower() == "modulo")
								mth.Operation = MathNode.Operations.Modulo;
						}
						nodes.Add(nodename, mth);
						shader.AddNode(mth);
						break;
					case "mapping":
						var mapping = new MappingNode();
						get_float4(mapping.Translation, node.GetAttribute("translation"));
						get_float4(mapping.Rotation, node.GetAttribute("rotation"));
						get_float4(mapping.Scale, node.GetAttribute("scale"));
						get_float4(mapping.Min, node.GetAttribute("min"));
						get_float4(mapping.Max, node.GetAttribute("max"));
						var vector_type = node.GetAttribute("vector_type");
						if (!string.IsNullOrEmpty(vector_type))
						{
							if (vector_type.ToLower() == "texture")
								mapping.vector_type = MappingNode.vector_types.TEXTURE;
							else if (vector_type.ToLower() == "point")
								mapping.vector_type = MappingNode.vector_types.POINT;
							else if (vector_type.ToLower() == "vector")
								mapping.vector_type = MappingNode.vector_types.VECTOR;
							else if (vector_type.ToLower() == "normal")
								mapping.vector_type = MappingNode.vector_types.NORMAL;
						}
					...
					case "emission":
						...
						get_float4(emnode.ins.Color, node.GetAttribute("color"));
						get_float(emnode.ins.Strength, node.GetAttribute("strength"));
						nodes.Add(nodename, emnode);
						shader.AddNode(emnode);
						break;
					case "connect":
		...
		public void ReadShader(ref XmlReadState state, System.Xml.XmlReader node)
		{
			...
			state.Scene.AddShader(shader);
			//state.Scene.DefaultSurface=shader;
		}
